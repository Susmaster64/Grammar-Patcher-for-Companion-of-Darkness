...
def create_venv_if_needed():
    ...
    if not os.path.exists(venv_dir):
        ...
        subprocess.check_call([pip_path, "install", "pandas", "openpyxl"])
...
def main():
    ...
    if sys.prefix != sys.base_prefix:
        ...
        def replace_lines(csv_file, txt_file):
            ...
            print('\nModified script exported to modscript.rpy\n')
            print(f"\nTotal lines processed: {total_lines}")
            print(f"\nLines found: {lines_found}")
            print(f"Number of lines not found: {total_lines - lines_found}")

            with open("modscript.rpy", "w") as file:
                for line in lines:
                    file.write(line)

            if total_lines - lines_found >= 25:
                print(
                    "\nCRITICAL: MORE THAN 25 LINES WEREN'T FOUND!\n"
